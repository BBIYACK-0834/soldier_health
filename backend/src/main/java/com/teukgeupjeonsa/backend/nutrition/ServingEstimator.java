...
public class ServingEstimator {
    ...
    public double estimateGram(String normalizedName, String category) {
        if (category != null && !category.isBlank()) {
            Optional<ServingDefault> exact = servingDefaultRepository.findFirstByCategory(category);
            if (exact.isPresent()) {
                return exact.get().getServingGram();
            }
        }
        ...
        if (containsAny(value, "면", "라면", "국수", "짜장면", "짬뽕")) return 350.0;
        if (containsAny(value, "소스", "양념", "드레싱")) return 30.0;
